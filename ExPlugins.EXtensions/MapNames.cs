...
public static class MapNames
{
	...
	public static readonly string ObasCursedTrove;
	...
	static MapNames()
	{
		...
		string text = "";
		FieldInfo[] fields = typeof(MapNames).GetFields();
		foreach (FieldInfo fieldInfo in fields)
		{
			AreaId customAttribute = fieldInfo.GetCustomAttribute<AreaId>();
			new Regex("\r\n                (?<=[A-Z])(?=[A-Z][a-z]) |\r\n                 (?<=[^A-Z])(?=[A-Z]) |\r\n                 (?<=[A-Za-z])(?=[^A-Za-z])", RegexOptions.IgnorePatternWhitespace);
			if (customAttribute != null)
			{
				if (!dictionary.TryGetValue(customAttribute.Id, out var value))
				{
					text = "Cannot initialize \"" + fieldInfo.Name + "\" field. DatWorldAreas does not contain area with \"" + customAttribute.Id + "\" id.";
					GlobalLog.Error("[MapNames] " + text);
				}
				else
				{
		...
		{
			BotManager.Stop(new StopReasonData("map_init_error", $"Unknown act: {World.CurrentArea.Act}.", (object)null), false);
		}
	}
}
